...
function resolvePluginPageUrl(path: string): string {
  const bridgeBase = new URLSearchParams(window.location.search).get("shinsekai_bridge")?.trim();
  if (!bridgeBase || !path.startsWith("/api/")) return path;
  try { return new URL(path, bridgeBase).toString(); } catch { return path; }
}
...
function directPage(target: PluginPageTarget): OverlayPage | null {
  ...
  return {
    src: resolvePluginPageUrl(target.frontendUrl),
  ...
  useEffect(() => {
    ...
    void getPluginUiDetail(target.pluginId)
      .then((detail) => {
        ...
        setPage({
          src: resolvePluginPageUrl(matchedPage.frontendUrl),
